...
#first tutorial https://www.youtube.com/watch?v=iRZBPDZ71Ak&feature=youtu.be
...
class Blockchain(object):
    ...
    def valid_proof(block_string, proof):
        ...
        return guess_hash[:3] == "000" #increase this to increase mining time aka finding the hash
...
def full_chain():
    response = {
       'chain': blockchain.chain,
        ...
        # Returns the chain and its current length
    }
    return jsonify(response), 200
...
class Blockchain(object):
    ...
    def new_block(self, proof, previous_hash=None):
        ...
        * The hash of the previous block
        ...
        block = {
            ...
            # Or 'previous_hash': self.hash(self.chain[-1])
    ...
    def hash(self, block):
        ...
        # Use json.dumps to convert json into a string
        string_block = json.dumps(block,
                                  sort_keys=True)  # sort_keys= True is a flag makes sure all of the keys in the dictionary
        # are turned into a string in A - Z order,
        # if the order changed so would the hash every time we ran the function and we don't want that
...
def mine():
    ...
    if 'id' and 'proof' in data:
        user_id = data['id']
        proof = data['proof']
        # Had to get help here, need to make a block_string
        block_string = json.dumps(blockchain.last_block, sort_keys= True) #must be the last block to validate that was the one mined

        if blockchain.valid_proof(block_string, proof):
            #Forge a new Block by adding it to the chain with proof
            previous_hash = blockchain.hash(blockchain.last_block)
            block = blockchain.new_block(proof, previous_hash)


            response = {
                'message': 'Yay you mined/forged a new block',
                'proof': block['proof']
            ...
            response = {
                'message': 'Not a valid proof or already submitted'
            }
        ...
        response = {
            'message': 'ERROR ERROR ALL ERRORS ERRORED WHEN TRYING TO ERROR EVERYTHING DUN ERRORED'
        }
